...
public class OldIntegrate {
    ...
    private static Map<String, Map<String, PsmInfo>> gPsmMap = new TreeMap<String, Map<String, PsmInfo>>();
...
//        TreeMap<String, List<String>> FileMap = LoadPsms(param.FileLi); //Store the psms, key: fName; value: psm list (0 is the title)
        ...
        Map<String, List<String>> FileMap = loader.loadPsmFiles(groupByEnum);
...
//            RemoveOutlier();
            ...
            try {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    if (isUsed) {
                        ...
                        if (groupkey != "null") {
                            if ((groupBy == 4) && (slpLi.size() > 0)) { //4. Single Phospho Site
    ...
    private static void GroupPsm(Map<String, List<String>> FileMap) throws IOException {
        for (String fName : FileMap.keySet()) {
            ds_Index indObj = param.indMap.get(fName);
            List<String> PsmLi = FileMap.get(fName);
            for (String Psm : PsmLi) {
                ...
                if (gPsmMap.containsKey(groupkey)) {
                    ...
                    TreeMap<String, PsmInfo> fMap = new TreeMap<String, PsmInfo>();
                    PsmInfo pi = new PsmInfo();
                    pi.gene = gene;
                    pi.peptide = NewPepSeq;
                    pi.extpep = ExtPepSeq;
    ...
    private static void RemoveOutlier() {
        for (String groupkey : gPsmMap.keySet()) {
            ...
            for (String fName : fMap.keySet()) {
                PsmInfo pi = fMap.get(fName);
                ds_Index indObj = param.indMap.get(fName);
                if (pi.psmList.size() > 3) { //only process pi with at least 4 psms, for speeding up
                    List<String> NewPsmLi = IqrPsm(pi.psmList, indObj);
                    pi.psmList.clear();
                    pi.psmList.addAll(NewPsmLi);
                }
            }
        }
    }
